...
def process_invoice_paid(invoice_data: dict):
    ...
    try:
        ...
        with get_db_session() as session:
            # SQL LIKE avoids loading all pending transactions into memory.
            transaction = session.query(Transaction).filter(
                Transaction.payment_method == 'crypto_wallet',
...
def _find_pending_transaction(method_value: str, gateway_ref: str):
    ...
    Uses a SQL LIKE predicate so the database filters rows rather than
    loading all pending transactions into Python memory.
    """
    with get_db_session() as session:
        txn = session.query(Transaction).filter(
            Transaction.payment_method == method_value,
            Transaction.status == TransactionStatus.PENDING,
            Transaction.crypto_address.like(f"{gateway_ref}|%"),
        ).first()
        return txn
