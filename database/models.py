...
class Product(Base):
    ...
    feature_items = relationship(
        ...
        back_populates="product",
        cascade="all, delete-orphan",
        order_by="ProductFeatureItem.display_order",
    )
...
# Phase 1 extension of the Product Information Builder (V48). Purely additive:
# does not touch payment flow, delivery logic, or existing callback_data.
# ══════════════════════════════════════════════════════════════════════════════

class ProductFeatureItem(Base):
    """A single row in a product's Feature Box (e.g. '✅ Instant Delivery').

    Unlimited rows per product, admin-managed, rendered in display_order ASC
    on the product detail card just below the price/status block.
    """
    __tablename__ = 'product_feature_items'
...
class ProductButtonSetting(Base):
    """Admin-configurable label/emoji/visibility/order for a shared product-page
    button (Buy Now, Back, Support, View Plans, Refresh, Favorite, Home).

    One row per ``button_key``, seeded with defaults by the admin panel on
    first access. Global by design — these buttons appear identically across
    every product page. Editing a row NEVER changes callback_data, only how
    the button is drawn.
    """
    __tablename__ = 'product_button_settings'

    id            = Column(Integer, primary_key=True)
    button_key    = Column(String(32),  nullable=False, unique=True, index=True)
...
class ProductTag(Base):
    """A reusable tag definition (Featured, Best Seller, New, ...).

    Admin can create/rename/delete tags freely; ``key`` is a stable internal
    slug while ``label``/``emoji`` control display. Not to be confused with
    the auto-computed badges in services/badges.py — these are manually
    assigned by the admin per product.
    """
    ...
    label         = Column(String(64),  nullable=False)
    emoji         = Column(String(32),  nullable=True)
    color         = Column(String(16),  nullable=True, default='none')
    is_active     = Column(Boolean, default=True, nullable=False)
    display_order = Column(Integer, default=0,    nullable=False)
    created_at    = Column(DateTime, default=datetime.utcnow, nullable=False)
...
class ProductTagLink(Base):
    """Many-to-many assignment of a ProductTag to a Product."""
    __tablename__ = 'product_tag_links'
    __table_args__ = (
        UniqueConstraint('product_id', 'tag_id', name='uq_product_tag_link'),
    )

    id         = Column(Integer, primary_key=True)
    product_id = Column(Integer, ForeignKey('products.id', ondelete='CASCADE'),
                        nullable=False, index=True)
    tag_id     = Column(Integer, ForeignKey('product_tags.id', ondelete='CASCADE'),
                        nullable=False, index=True)
    created_at = Column(DateTime, default=datetime.utcnow, nullable=False)
