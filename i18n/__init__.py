...
def prime_user_language(telegram_id: int) -> str:
    """Read a user's language from DB and cache it.

    This is intended for a worker thread (the bot activity middleware uses
    it there). Keeping the normal getter cache-first prevents repeated
    synchronous DB calls from every command and callback.
    """
    try:
        ...
        with get_db_session() as session:
            ...
            lang = normalize_lang(u.language) if u else DEFAULT_LANGUAGE
    except Exception as e:
        print(f"[i18n] prime_user_language failed: {e}")
        lang = DEFAULT_LANGUAGE
    _user_language_cache[telegram_id] = (lang, time.monotonic())
    return lang
...
def get_user_language(telegram_id: int) -> str:
    ...
    if cached and time.monotonic() - cached[1] < _USER_LANGUAGE_TTL:
        return cached[0]
    try:
        return prime_user_language(telegram_id)
    except Exception as e:
