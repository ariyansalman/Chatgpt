...
from utils.menu_registry import DEFAULT_MENU_ITEMS
from utils.button_colors import DEFAULT_MENU_ITEM_COLORS, telegram_style_for_color
...
# The built-in Main Menu items' default emoji/color are owned by
# utils.menu_registry.DEFAULT_MENU_ITEMS and utils.button_colors, respectively
# -- these seed-default rows read from those tables instead of keeping their
# own hand-copied values, so they can never silently drift from the real
# menu's defaults.
_MENU_ITEM_EMOJI = {item["key"]: item.get("emoji", "") for item in DEFAULT_MENU_ITEMS}
_MENU_ITEM_STYLE = {
...
DEFAULTS: List[Tuple[str, str, Any, str, str, str]] = [
    ...
    ("notif_show_telegram_id", "bool", False, "notifications",
     "Show Telegram ID",
     "Show the customer's Telegram ID on admin order/payment notifications. Off by default for privacy."),
    ...
    ("global_button_colors_enabled", "bool", True, "main_menu",
     ...
     "this switch covers."),
    ("button_color_overrides_json", "text", "{}", "main_menu",
     "🎨 Button Color Overrides",
     "Persistent per-button color selections used by the dynamic color "
     "manager. This is presentation metadata only."),
    ("menu_item_products_enabled", "bool", True, "main_menu",
     "📋 Menu Item: Products",
     "Whether the 🛒 Products button remains actionable."),
    ("menu_item_products_visible", "bool", True, "main_menu",
     "👁 Menu Item: Products Visibility",
     "Show or hide the Products button."),
    ...
    ("menu_item_products_audience", "str", "all", "main_menu",
     "👥 Menu Item: Products Audience",
     "Audience: all, admin, user, or premium."),
    ("menu_item_topup_enabled", "bool", True, "main_menu",
     "📋 Menu Item: Top Up",
     "Whether the 💰 Top Up button remains actionable."),
    ("menu_item_topup_visible", "bool", True, "main_menu",
     "👁 Menu Item: Top Up Visibility",
     "Show or hide the Top Up button."),
    ...
    ("menu_item_topup_audience", "str", "all", "main_menu",
     "👥 Menu Item: Top Up Audience",
     "Audience: all, admin, user, or premium."),
    ("menu_item_wallet_enabled", "bool", True, "main_menu",
     "📋 Menu Item: Wallet",
     "Whether the Wallet button remains actionable."),
    ("menu_item_wallet_visible", "bool", True, "main_menu",
     "👁 Menu Item: Wallet Visibility",
     "Show or hide the Wallet button."),
    ...
    ("menu_item_orders_enabled", "bool", True, "main_menu",
     "📋 Menu Item: Orders",
     "Whether the 📜 Orders button remains actionable."),
    ("menu_item_orders_visible", "bool", True, "main_menu",
     "👁 Menu Item: Orders Visibility",
     "Show or hide the Orders button."),
    ...
    ("menu_item_support_audience", "str", "all", "main_menu",
     "👥 Menu Item: Support Audience",
     "Audience: all, admin, user, or premium."),
    ("menu_item_refer_enabled", "bool", True, "main_menu",
     "📋 Menu Item: Refer & Earn",
     "Whether the 🎁 Refer & Earn button remains actionable."),
    ("menu_item_refer_visible", "bool", True, "main_menu",
     "👁 Menu Item: Refer & Earn Visibility",
     "Show or hide the Refer & Earn button."),
    ...
    ("menu_item_refer_audience", "str", "all", "main_menu",
     "👥 Menu Item: Refer & Earn Audience",
     "Audience: all, admin, user, or premium."),
    ("menu_item_account_enabled", "bool", True, "main_menu",
     "📋 Menu Item: Account",
     "Whether the 👤 Account button remains actionable."),
    ("menu_item_account_visible", "bool", True, "main_menu",
     "👁 Menu Item: Account Visibility",
     "Show or hide the Account button."),
    ...
    ("menu_item_account_audience", "str", "all", "main_menu",
     "👥 Menu Item: Account Audience",
     "Audience: all, admin, user, or premium."),
    ("menu_item_language_enabled", "bool", True, "main_menu",
     "📋 Menu Item: Language",
     "Whether the 🌐 Language button remains actionable."),
    ("menu_item_language_visible", "bool", True, "main_menu",
     "👁 Menu Item: Language Visibility",
    ...
    ("menu_item_language_audience", "str", "all", "main_menu",
     "👥 Menu Item: Language Audience",
     "Audience: all, admin, user, or premium."),
    ("menu_item_admin_enabled", "bool", True, "main_menu",
     "📋 Menu Item: Admin Panel",
     "Whether the Admin Panel button remains actionable."),
    ("menu_item_admin_visible", "bool", True, "main_menu",
     "👁 Menu Item: Admin Panel Visibility",
     "Show or hide the Admin Panel button."),
    ...
    ("menu_item_admin_audience", "str", "admin", "main_menu",
     "👥 Menu Item: Admin Panel Audience",
     "Audience: all, admin, user, or premium."),
    ...
    ("main_menu_items_json", "text", "[]", "main_menu",
     "📋 Menu Items Layout (JSON)",
     "Internal layout override managed by Menu Manager. Do not edit manually."),
    ("main_menu_layout_users_json", "text", "", "main_menu",
     "👥 Users Menu Layout (JSON)",
     "Complete presentation profile for regular users. Managed by Menu Manager."),
    ("main_menu_layout_premium_json", "text", "", "main_menu",
     "⭐ Premium Users Menu Layout (JSON)",
     "Complete presentation profile for active subscribers. Managed by Menu Manager."),
    ("main_menu_layout_admins_json", "text", "", "main_menu",
     "🛠 Admins Menu Layout (JSON)",
     "Complete presentation profile for admins. Managed by Menu Manager."),
    ("main_menu_layout_saved_json_users", "text", "", "main_menu",
     "💾 Users Saved Menu Layout (JSON)",
     "Last saved layout snapshot for the Users profile."),
    ("main_menu_layout_saved_json_premium", "text", "", "main_menu",
     "💾 Premium Users Saved Menu Layout (JSON)",
     "Last saved layout snapshot for the Premium Users profile."),
    ("main_menu_layout_saved_json_admins", "text", "", "main_menu",
     "💾 Admins Saved Menu Layout (JSON)",
     "Last saved layout snapshot for the Admins profile."),
...
class _BotConfigCache:
    ...
    def _ensure_fresh(self) -> None:
        if not self._cache:
            # Startup calls preload() before Telegram starts. Keep this
            # synchronous fallback for scripts/tests that use cfg directly.
            self._load()
            return
        if (time.time() - self._loaded_at) > self._CACHE_TTL:
            # Stale-while-revalidate: never make a message wait for a config
            # query. A daemon thread refreshes the cache while callers use
            ...
            def _refresh() -> None:
                try:
                    ...
                    logger.exception("Failed to refresh bot_config cache")
                finally:
                    with self._refresh_lock:
                        self._refresh_inflight = False
            ...
            threading.Thread(
                target=_refresh,
                name="bot-config-refresh",
                daemon=True,
            ).start()

    def preload(self) -> None:
        """Load config once during startup, before Telegram handles updates."""
        self._load()
