...
async def end_conversation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Generic fallback callback that just ends the conversation.

    Use this instead of ``lambda u, c: ConversationHandler.END`` as a
    ConversationHandler entry/state/fallback callback. PTB always does
    ``await self.callback(update, context)`` on handler callbacks, and a
    plain sync lambda returns a bare int instead of a coroutine, which
    raises ``TypeError: object int can't be used in 'await' expression``.
    If the update came from a callback query, it is also answered so the
    button doesn't show a loading spinner/timeout on the client.
    """
    if update.callback_query:
        try:
            await update.callback_query.answer()
