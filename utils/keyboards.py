...
def create_product_detail_keyboard(
    ...
    if stock_count == 0:
        ...
        # V49: Button Builder — admin-controlled label/emoji/visibility.
        # callback_data is never affected by these settings.
        try:
            from services.button_builder_service import is_button_visible, button_text
            _buy_visible = is_button_visible("buy_now")
            _buy_text = button_text("buy_now")
        except Exception:
            _buy_visible, _buy_text = True, "🛒 Buy Now"
        keyboard = []
        if _buy_visible:
            keyboard.append([InlineKeyboardButton(_buy_text, callback_data=f"buy_{product_id}")])
    ...
    if telegram_id is not None:
        ...
        try:
            ...
            from services.button_builder_service import is_button_visible as _btn_vis
            if _cfg2.get_bool("feature_favorites_enabled", True) and _btn_vis("favorite"):
    ...
    # V49: Button Builder — admin-controlled label/emoji/visibility for the
    # Back and Support buttons. callback_data is never affected.
    try:
        from services.button_builder_service import is_button_visible, button_text
        _back_visible, _support_visible = is_button_visible("back"), is_button_visible("support")
    ...
    if _support_visible:
        _last_row.append(InlineKeyboardButton(_support_text, callback_data="support_center"))
    if _last_row:
        keyboard.append(_last_row)
    return InlineKeyboardMarkup(keyboard)
