...
def format_order_id(order_id: int, created_at=None) -> str:
    ...
    """
    dt = created_at if created_at is not None else datetime.utcnow()
...
_BAN_CACHE_MAX_SIZE = 5000  # evict oldest entries once the dict exceeds this


def _evict_ban_cache() -> None:
    """Drop the oldest half of entries when the cache grows too large."""
    global _ban_cache
    if len(_ban_cache) <= _BAN_CACHE_MAX_SIZE:
        return
    # Sort by timestamp (index 1 of the tuple) and keep the newest half.
    sorted_items = sorted(_ban_cache.items(), key=lambda kv: kv[1][1])
    keep_from = len(sorted_items) // 2
    _ban_cache = dict(sorted_items[keep_from:])
...
def check_user_banned(telegram_id: int) -> bool:
    ...
    with get_db_session() as session:
        ...
        # Evict stale/excess entries before inserting a new one.
        _evict_ban_cache()
