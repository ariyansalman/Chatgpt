...
from utils.safe_conversation import cancel_command, end_conversation
...
async def _track_activity(update, context):
    ...
    try:
        ...
        # Activity is useful, but it does not need to query PostgreSQL on
        # every update. This also prevents a burst of Telegram updates from
        # queueing one DB worker per message.
        now_mono = time.monotonic()
        if now_mono - _activity_checked_at.get(user.id, 0.0) < _ACTIVITY_CHECK_TTL:
            return
        _activity_checked_at[user.id] = now_mono

        def _touch_and_track():
            """Run every synchronous DB operation in one worker thread."""
            with get_db_session() as s:
                ...
                if not row.last_seen_at or (now - row.last_seen_at) >= throttle:
                    if not row.last_seen_at or (now - row.last_seen_at) > timedelta(hours=12):
                        is_new_session = True
                    row.last_seen_at = now

                # Prime the language cache away from the event loop.
                try:
                    from i18n import prime_user_language
                    prime_user_language(user.id)
                except Exception:
                    pass

                # V19 — session + login activity tracking.
                try:
                ...
                # V32 — login activity/device tracking.
                if is_new_session:
                    try:
                        from services.login_activity import record_login
                        return record_login(user, row.id)
                    except Exception:
                        logger.debug("V32 login tracking failed", exc_info=True)
            return None

        login_result = await asyncio.to_thread(_touch_and_track)
        if login_result:
            try:
                from services.login_activity import send_new_login_alert
                _rid, _is_nd, _is_sus = login_result
                asyncio.create_task(
                ...
                logger.debug("new-login alert failed", exc_info=True)
...
def main():
    ...
    try:
        seed_defaults()
        # Warm the config cache before Telegram starts. Later refreshes use
        # stale-while-revalidate and never hold up an incoming update.
        cfg.preload()
    except Exception:
        logger.exception("BotConfig seeding/cache warm-up failed")

    # Auto-sync Main Menu profiles (Users/Premium/Admins) to the current
    # DEFAULT_MENU_ITEMS whenever MENU_DEFAULTS_VERSION was bumped in this
    # deploy. Idempotent -- a no-op on restarts where nothing changed.
    try:
        from utils.menu_registry import sync_menu_defaults_on_startup
        sync_menu_defaults_on_startup()
    except Exception:
    ...
    topup_conv_handler = ConversationHandler(
        ...
        states={
            ...
            payment_handlers.METHOD: [
                ...
                CallbackQueryHandler(payment_handlers.topup_back_to_amount_selection, pattern="^topup_back_to_amount$"),
                # "▶️ Continue Deposit" on the Pending Deposit notice (every
                # gateway: CryptoBot, bKash/Nagad/Cryptomus/NOWPayments,
                # ZiniPay, Binance Pay, Bybit Pay) — reopens the existing
                # pending order's payment screen; never creates a new deposit.
            ...
            payment_handlers.MANUAL_TXID: [
                ...
                CallbackQueryHandler(payment_handlers.cancel_topup, pattern="^back_payment_methods$"),
            ],
        ...
        fallbacks=[
            ...
            # prompts). Registered as a fallback (not per-state) so it
            # fires no matter which state the user is currently in. See
            # payment_handlers.deposit_cancel / services/payment_ui.py:
            # with_deposit_cancel.
    ...
    # Bug this fixes: Back (payment_handlers.cancel_payment_page, "^back_payment_methods$")
    ...
    # ended the conversation, so — same reasoning as every other button in
    # this standalone block — it needs its own registration here too, not
    # just the one inside topup_conv_handler's AMOUNT_SELECT state.
    application.add_handler(CallbackQueryHandler(payment_handlers.topup_back_to_wallet, pattern="^topup_back_to_wallet$"))
    ...
    # (topup_conv_handler / binance_submit_conv / bybit_submit_conv /
    # zinipay_submit_conv) for while a conversation is active, plus this
    # standalone handler for screens shown after their conversation has
    # already ended. See payment_handlers.deposit_cancel.
    application.add_handler(CallbackQueryHandler(payment_handlers.deposit_cancel, pattern="^deposit_cancel$"))
    ...
    application.add_handler(CallbackQueryHandler(admin_handlers.admin_reactivate_order_callback, pattern=r"^reactivate_order_\d+$"))
    ...
    application.add_handler(CallbackQueryHandler(admin_zinipay.admin_zinipay_toggle_provider,
                                                  pattern="^admin_zinipay_toggle_provider_"))
    ...
    cfg_search_conv = ConversationHandler(
        ...
        states={
            admin_cfg.SEARCH_QUERY: [
                ...
                CallbackQueryHandler(end_conversation, pattern=r"^admin_bot_config$"),
            ],
        },
    ...
    # Register text-entry conversations BEFORE the generic mm dispatcher so
    # they intercept the relevant entry-point callbacks first.
    application.add_handler(admin_menu_manager.build_mm_edit_conversation())
    application.add_handler(admin_menu_manager.build_custom_button_conversation())
    # V20+: Admin Menu Builder — rename, emoji, callback, audience, add item,
    # create menu, delete item/menu conversations. Must be registered before the
    # generic mm: dispatcher so its entry-point patterns take priority.
    application.add_handler(admin_menu_manager.build_amgr_conversation())
    application.add_handler(CallbackQueryHandler(admin_menu_manager.mm_dispatch, pattern=r"^mm:"))
    application.add_handler(
        CallbackQueryHandler(user_handlers.menu_disabled_callback, pattern=r"^menu_disabled:")
    )
    ...
    # V49: Premium Product System — Feature Box, Button Builder, Product Tags
    from handlers.admin_feature_box import register_handlers as _fbx_register
    _fbx_register(application)
    from handlers.admin_button_builder import register_handlers as _btnb_register
    _btnb_register(application)
    from handlers.admin_product_tags import register_handlers as _ptag_register
    _ptag_register(application)
