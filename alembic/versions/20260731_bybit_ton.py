"""bybit_ton: add USDT TON deposit address support to Bybit Pay gateway.

Revision ID: 20260731_bybit_ton
Revises: 20260730_bybit_avaxc

Adds:
  - bybit_wallet_ton column to payment_gateway_configs (USDT TON deposit address)
...
revision = "20260731_bybit_ton"
down_revision = "20260730_bybit_avaxc"
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    if not sa_inspect(bind).has_table("payment_gateway_configs"):
        logger.warning(
            "payment_gateway_configs table not found — skipping bybit_ton migration."
        )
        return

    # Add the USDT TON deposit address column alongside TRC20/BEP20/ERC20/LTC/AVAXC.
    _safe_add_column("payment_gateway_configs", "bybit_wallet_ton", sa.String(255))
