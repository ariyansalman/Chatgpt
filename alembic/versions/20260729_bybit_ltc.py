...
def upgrade():
    ...
    if not sa_inspect(bind).has_table("payment_gateway_configs"):
        logger.warning(
            "payment_gateway_configs table not found — skipping bybit_ltc migration."
        )
        return

    # Add the LTC deposit address column alongside the existing TRC20/BEP20/ERC20 columns.
    _safe_add_column("payment_gateway_configs", "bybit_wallet_ltc", sa.String(255))
