"""bybit_arb: add USDT Arbitrum One deposit address support to Bybit Pay gateway.

Revision ID: 20260803_bybit_arb
Revises: 20260802_bybit_base

Adds:
  - bybit_wallet_arb column to payment_gateway_configs (USDT Arbitrum One deposit address)
...
revision = "20260803_bybit_arb"
down_revision = "20260802_bybit_base"
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    if not sa_inspect(bind).has_table("payment_gateway_configs"):
        logger.warning(
            "payment_gateway_configs table not found — skipping bybit_arb migration."
        )
        return

    # Add the USDT Arbitrum One deposit address column alongside TRC20/BEP20/ERC20/LTC/AVAXC/TON/BASE.
    _safe_add_column("payment_gateway_configs", "bybit_wallet_arb", sa.String(255))
