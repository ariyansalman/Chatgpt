"""bybit_sol: add USDT Solana deposit address support to Bybit Pay gateway.

Revision ID: 20260806_bybit_sol
Revises: 20260805_bybit_matic

Adds:
  - bybit_wallet_sol column to payment_gateway_configs (USDT Solana deposit address)
...
revision = "20260806_bybit_sol"
down_revision = "20260805_bybit_matic"
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    if not sa_inspect(bind).has_table("payment_gateway_configs"):
        logger.warning(
            "payment_gateway_configs table not found — skipping bybit_sol migration."
        )
        return

    # Add the USDT Solana deposit address column alongside TRC20/BEP20/ERC20/LTC/AVAXC/TON/BASE/ARBONE/OP/MATIC.
    _safe_add_column("payment_gateway_configs", "bybit_wallet_sol", sa.String(255))
