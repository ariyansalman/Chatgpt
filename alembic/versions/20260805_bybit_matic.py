"""bybit_matic: add USDT Polygon (MATIC) deposit address support to Bybit Pay gateway.

Revision ID: 20260805_bybit_matic
Revises: 20260804_bybit_op

Adds:
  - bybit_wallet_matic column to payment_gateway_configs (USDT Polygon deposit address)
...
revision = "20260805_bybit_matic"
down_revision = "20260804_bybit_op"
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    if not sa_inspect(bind).has_table("payment_gateway_configs"):
        logger.warning(
            "payment_gateway_configs table not found — skipping bybit_matic migration."
        )
        return

    # Add the USDT Polygon deposit address column alongside TRC20/BEP20/ERC20/LTC/AVAXC/TON/BASE/ARBONE/OP.
    _safe_add_column("payment_gateway_configs", "bybit_wallet_matic", sa.String(255))
