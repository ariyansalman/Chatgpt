...
def get_broadcast_dashboard_stats() -> Dict[str, Any]:
    ...
    with get_db_session() as s:
        ...
        # Single GROUP BY query replaces 8 separate COUNT queries.
        status_rows = (
            s.query(ScheduledBroadcast.status, func.count(ScheduledBroadcast.id))
            .group_by(ScheduledBroadcast.status)
            .all()
        )
        _sc = {row[0]: row[1] for row in status_rows}
        total     = sum(_sc.values())
        running   = _sc.get("sending",   0)
        scheduled = _sc.get("scheduled", 0)
        paused    = _sc.get("paused",    0)
        completed = _sc.get("sent",      0)
        failed    = _sc.get("failed",    0)
        cancelled = _sc.get("cancelled", 0)
        drafts    = _sc.get("draft",     0)
