...
def get_campaign_dashboard_stats() -> Dict[str, Any]:
    ...
    with get_db_session() as s:
        # Single GROUP BY query replaces 7 separate COUNT queries on BroadcastCampaign.
        status_rows = (
            s.query(BroadcastCampaign.status, BroadcastCampaign.is_archived,
                    func.count(BroadcastCampaign.id))
            .group_by(BroadcastCampaign.status, BroadcastCampaign.is_archived)
            .all()
        )
        _sc: Dict[str, int] = {}
        archived = 0
        for _status, _is_arch, _cnt in status_rows:
            if _is_arch:
                archived += _cnt
            else:
                _sc[_status] = _cnt
        total     = sum(_sc.values())
        drafts    = _sc.get(CampaignStatus.DRAFT.value,      0)
        scheduled = _sc.get(CampaignStatus.SCHEDULED.value,  0)
        running   = _sc.get(CampaignStatus.RUNNING.value,    0)
        paused    = _sc.get(CampaignStatus.PAUSED.value,     0)
        completed = _sc.get(CampaignStatus.COMPLETED.value,  0)
        cancelled = _sc.get(CampaignStatus.CANCELLED.value,  0)

        # Templates: two counts become one query + arithmetic.
        ...
        # Automation: two counts become one query + arithmetic.
