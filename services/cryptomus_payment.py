...
class CryptomusPaymentService:
    ...
    def create_payment(
        ...
        _lifetime_seconds = max(1, cfg.get_int("payment_expiry_minutes", 30)) * 60
        payload = {
            "amount": f"{amount:.2f}",
            "currency": "USD",
            "order_id": str(transaction_id),
            "lifetime": str(_lifetime_seconds),  # admin-configurable via bot_config.payment_expiry_minutes
        }
        if callback_url:
