...
def format_product_detail_card(product, telegram_id: int = None) -> str:
    ...
    # ── Product Tags (Premium Product System, Feature 7) ───────────────────
    try:
        from services.product_tags_service import render_tag_line
        _tag_line = render_tag_line(product.id)
        if _tag_line:
    ...
    # ── Feature Box (Premium Product System, Feature 2) ────────────────────
    try:
        from services.feature_box_service import render_feature_box_html
        _fbox = render_feature_box_html(product.id)
        if _fbox:
            lines.append("─────────────────────────────────")
            lines.append(_fbox)
    except Exception:
