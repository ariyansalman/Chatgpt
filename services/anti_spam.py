...
def check_access(tg_id: int) -> Tuple[bool, Tuple[bool, str]]:
    """Resolve whitelist and moderation state in one worker-thread call."""
    _refresh_list_caches()
    if tg_id in _wl_cache:
        return True, (True, "")
    return False, can_interact(tg_id)


# ─── Rate limiting ────────────────────────────────────────────────────────────

_BUCKET_CLEANUP_INTERVAL = 300   # seconds between stale-user sweeps
_bucket_last_cleanup: float = 0.0


def _cleanup_buckets() -> None:
    """Remove entries for users who have been silent for > 2× the flood window.
    Prevents _buckets from growing without bound in long-running processes.
    """
    global _bucket_last_cleanup
    now = time.monotonic()
    if now - _bucket_last_cleanup < _BUCKET_CLEANUP_INTERVAL:
        return
    _bucket_last_cleanup = now
    max_window = (_cfg_int("antispam_flood_window_secs") or 10) * 2
    cutoff = now - max_window
    for bucket_name in list(_buckets):
        bucket = _buckets[bucket_name]
        stale_users = [uid for uid, dq in bucket.items() if not dq or dq[-1] < cutoff]
...
def _push_event(tg_id: int, bucket: str) -> int:
    ...
    while dq and dq[0] < cutoff:
        dq.popleft()
    # Periodic sweep to evict entries for long-silent users.
    _cleanup_buckets()
    return len(dq)
...
async def antispam_middleware(update, context) -> None:
    ...
    # Both the cache refresh and the moderation query can touch PostgreSQL.
    # Keep the complete access check off the asyncio event loop.
    from database import run_db
    whitelisted, access = await run_db(check_access, tg_id)
    if whitelisted:
        return
    ok, reason = access
    if not ok:
    ...
    if update.message:
        ...
        if check_flood(tg_id):
            await run_db(record_violation, tg_id, username,
        ...
        # Rate limit
            ...
            await run_db(record_violation, tg_id, username, vtype,
