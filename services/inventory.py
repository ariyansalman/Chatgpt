...
def _expire_stale(session) -> int:
    ...
    Uses two bulk SQL statements instead of one per reservation, reducing
    ...
    # Single bulk UPDATE to release all locked keys at once.
    session.query(ProductKey).filter(
        ProductKey.reservation_id.in_(stale_ids),
        ProductKey.is_sold == False,  # noqa: E712
    ).update({"reservation_id": None}, synchronize_session=False)

    # Mark all stale reservations expired in one pass.
    for r in stale:
