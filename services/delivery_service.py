...
def deliver_bundle(session, order, item, product):
    ...
    # Pre-fetch ALL child products in one IN query instead of one query per child.
    child_ids = [bi.child_product_id for bi in bundle_children]
    _children_map: dict = {
        p.id: p
        for p in session.query(Product).filter(Product.id.in_(child_ids)).all()
    }

    # Prevent recursion — reject bundle-in-bundle chains.
    for bi in bundle_children:
        child = _children_map.get(bi.child_product_id)
        if not child:
            return DeliveryResult(handled=True, success=False,
    ...
    for bi in bundle_children:
        ...
        child = _children_map[bi.child_product_id]
