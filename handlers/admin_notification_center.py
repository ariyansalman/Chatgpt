...
async def anc_list(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton("« Prev", callback_data=f"anc:list:{filt}:{page-1}"))
...
async def anc_settings(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Mark the currently-active state with ● ; inactive states with ○.
    def _ns_btn(icon: str, label: str, val: str) -> InlineKeyboardButton:
        mark = "●" if status == val else "○"
        return InlineKeyboardButton(
            f"{icon} {mark} {label}",
            callback_data=f"anc:settings:status:{val}",
        )

    btns = [
        [
            _ns_btn("🟢", "Enabled",     "enabled"),
            _ns_btn("🟡", "Maintenance", "maintenance"),
            _ns_btn("🔴", "Disabled",    "disabled"),
        ],
    ]
