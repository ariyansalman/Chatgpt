...
def _deposit_kb(tx, user=None, prev_tx_id=None, next_tx_id=None) -> InlineKeyboardMarkup:
...
async def deposit_detail(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with get_db_session() as session:
        ...
        kb = _deposit_kb(tx, u, prev_tx_id=prev_tx_id, next_tx_id=next_tx_id)
...
async def deposit_approve_execute(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if remaining_ids:
        ...
        with get_db_session() as session:
            ...
            next_kb  = (
                _deposit_kb(next_tx, next_u, prev_tx_id=prev_tx_id, next_tx_id=next_tx_id)
                if next_tx else
                InlineKeyboardMarkup([[InlineKeyboardButton("⬅ Back", callback_data=_BACK_TO_LIST_CB)]])
            )
        await _safe_edit(
