...
def _kb_nav(page: int, has_next: bool, extra_rows: list = None) -> InlineKeyboardMarkup:
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton("« Prev", callback_data=f"acc:audit:page:{page-1}"))
    if has_next:
        nav.append(InlineKeyboardButton("Next »", callback_data=f"acc:audit:page:{page+1}"))
    kb = []
    if nav:
        kb.append(nav)
    if extra_rows:
...
def build_audit_search_conv() -> ConversationHandler:
    return ConversationHandler(
        ...
        fallbacks=[
            CallbackQueryHandler(end_conversation, pattern=r"^acc:audit:page:0$"),
        ],
