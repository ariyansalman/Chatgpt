...
async def aim_list(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton("« Prev", callback_data=f"aim:list:{page-1}"))
    if page < total_pages - 1:
        nav.append(InlineKeyboardButton("Next »", callback_data=f"aim:list:{page+1}"))
    if nav:
        buttons.append(nav)
    buttons.append([
        InlineKeyboardButton("➕ Add New", callback_data="aim:add"),
...
async def aim_logs(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton("« Prev", callback_data=f"aim:logs:{iid}:{page-1}"))
    if page < total_pages - 1:
        nav.append(InlineKeyboardButton("Next »", callback_data=f"aim:logs:{iid}:{page+1}"))
    buttons = []
    if nav:
        buttons.append(nav)
