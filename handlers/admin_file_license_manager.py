...
async def flm_files_list(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton("« Prev", callback_data=f"flm:files:list:{page-1}"))
...
async def flm_keys_list(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton("« Prev", callback_data=f"flm:keys:list:{kt}:{status}:{page-1}"))
    total_pages = (total + _PAGE_SIZE - 1) // _PAGE_SIZE
