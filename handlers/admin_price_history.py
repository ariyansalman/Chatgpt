...
async def price_history_manager(update, context, page: int = 0):
    ...
    if pages > 1:
        nav = []
        if page > 0:
            nav.append(InlineKeyboardButton("« Prev", callback_data=f"acc:ph:mgr:{page - 1}"))
        nav.append(InlineKeyboardButton(f"{page + 1}/{pages}", callback_data="acc:noop"))
        if page < pages - 1:
            nav.append(InlineKeyboardButton("Next »", callback_data=f"acc:ph:mgr:{page + 1}"))
        kb.append(nav)
...
async def route(action: str, rest: list, update, context):
    ...
    if action == "prod" and rest:
        ...
        if pages > 1:
            nav = []
            if page > 0:
                nav.append(InlineKeyboardButton("« Prev", callback_data=f"acc:ph:prod:{pid}:{page - 1}"))
            nav.append(InlineKeyboardButton(f"{page + 1}/{pages}", callback_data="acc:noop"))
            if page < pages - 1:
                nav.append(InlineKeyboardButton("Next »", callback_data=f"acc:ph:prod:{pid}:{page + 1}"))
            kb_rows.append(nav)
