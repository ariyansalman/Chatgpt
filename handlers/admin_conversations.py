...
from utils import format_price, create_admin_product_menu_keyboard, create_admin_category_menu_keyboard
...
async def product_price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # V11 — 12-option paginated product-type picker.
        from handlers.admin_product_types import build_type_picker
        await update.message.reply_text(
            "📦 Select product type:",
            reply_markup=build_type_picker(page=0),
        )
        return PRODUCT_TYPE
...
async def product_type(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle product type selection (V11 — 12 types, paginated picker)."""
    from handlers.admin_product_types import build_type_picker, parse_type_callback
    ...
    # Pagination — redraw picker at requested page.
    if query.data.startswith("ptype_page:"):
        try:
        ...
        try:
            await query.edit_message_text(
                "📦 Select product type:",
                reply_markup=build_type_picker(page=page),
            )
        except BadRequest as e:
            if "Message is not modified" not in str(e):
                raise
    ...
    resolved = parse_type_callback(query.data)
    if resolved is None:
        # Unknown callback — redraw picker.
        try:
            await query.edit_message_text(
                "📦 Select product type:",
                reply_markup=build_type_picker(page=0),
    ...
    context.user_data['product_type'] = resolved
    ...
    with get_db_session() as session:
        ...
        if not categories:
            # No categories exist yet — category_id is optional on Product,
            # so just skip straight to the image step instead of blocking
            # product creation on having a category first.
            context.user_data['product_category'] = None
...
async def _prompt_next_after_image(update, context):
    """After the (optional) image, branch based on product type.

    V11 — KEY / REDEEM_LINK / ACCOUNT_LOGIN / VOUCHER share the same
    ``product_keys`` inventory upload UI. FILE / DOWNLOADABLE_FILE prompt
    for a download link. Everything else (auto-gen, manual, pre-order,
    subscription, bundle, service, external) skips straight to product
    creation — per-type configuration can be edited from the admin
    control center once the product exists.
    """
    cancel_keyboard = [[InlineKeyboardButton("❌ Cancel", callback_data="cancel_product")]]
    ...
    FILE_LIKE = {ProductType.FILE, ProductType.DOWNLOADABLE_FILE}

    if ptype in FILE_LIKE:
        await update.message.reply_text(
            "🔗 Please enter the download link (or Telegram file URL) for this product:",
            reply_markup=InlineKeyboardMarkup(cancel_keyboard),
        )
        return PRODUCT_DOWNLOAD_LINK

    if ptype in KEY_LIKE:
        label_map = {
            ProductType.KEY: "🔑 Please paste the product KEYS (one per line) or upload a .txt file",
            ProductType.REDEEM_LINK: "🔗 Please paste the REDEEM LINKS (one per line) or upload a .txt file",
            ProductType.ACCOUNT_LOGIN: (
                "📧 Please paste the ACCOUNTS (one per line) — supported formats:\n"
                "  email|password\n"
                "  email|password|recovery_email\n"
                "  email|password|recovery_email|recovery_code\n"
                "  Email: user@example.com\n  Pw: Password123\n  2fa: abcd efgh ijkl mnop\n"
                "…or upload a .txt file"
            ),
            ProductType.VOUCHER: "🎟️ Please paste the VOUCHER CODES (one per line) or upload a .txt file",
        }
        await update.message.reply_text(
            label_map[ptype] + "\n\nOr type 'skip' to add inventory later.",
            reply_markup=InlineKeyboardMarkup(cancel_keyboard),
        )
        return PRODUCT_KEYS

    # All other V11 types — create product straight away; per-type config
    # (plans, bundle items, integrations, generation settings, etc.) is
    # edited afterwards from the admin control center.
...
async def product_image(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.text and update.message.text.lower() == 'skip':
        ...
        return await _prompt_next_after_image(update, context)
        ...
        # Admin uploaded a file during the image step — guide them
        await update.message.reply_text(
            "📎 File received, but this step expects a *product image* (jpg/png).\n"
            "Type *skip* to skip the image, then you can upload your inventory file "
            "in the next step.",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup(cancel_keyboard)
        )
        ...
        await update.message.reply_text(
            "❌ Please send a product image, or type 'skip' to skip:",
            reply_markup=InlineKeyboardMarkup(cancel_keyboard)
        )
        return PRODUCT_IMAGE
...
async def product_keys(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle product keys / inventory input during product creation.

    Accepts:
      - .txt file upload (UTF-8, UTF-8 BOM, CRLF/LF)
      - pasted text (one item per line)
      - 'skip' to defer inventory

    Each non-empty line becomes one inventory item (key, link, account, voucher).
    Lines are NOT split on ':' or any other delimiter.
    On error, stays in PRODUCT_KEYS state and shows a helpful message.
    """

    cancel_keyboard = [[InlineKeyboardButton("❌ Cancel", callback_data="cancel_product")]]

    # Build a dynamic label based on product type
    ptype = context.user_data.get('product_type')
    type_hint_map = {
        ProductType.KEY:           "🔑 key",
        ProductType.REDEEM_LINK:   "🔗 redeem link",
        ProductType.ACCOUNT_LOGIN: "📧 account/login entry",
        ProductType.VOUCHER:       "🎟 voucher code",
    }
    item_label = type_hint_map.get(ptype, "item")
    ...
    if update.message and update.message.document:
        ...
        try:
            ...
            if document.file_size and document.file_size > MAX_BYTES:
                await update.message.reply_text(
                    f"❌ File too large ({document.file_size // 1024} KB). "
                    f"Maximum is 5 MB. Please split into smaller files and try again.",
                    reply_markup=InlineKeyboardMarkup(cancel_keyboard)
                )
                return PRODUCT_KEYS
            ...
            # Decode: try UTF-8 with BOM stripping first, then latin-1 fallback
            try:
                text = raw.decode('utf-8-sig')  # strips UTF-8 BOM automatically
            except UnicodeDecodeError:
                try:
                    text = raw.decode('latin-1')
                except Exception:
                    await update.message.reply_text(
                        "❌ Could not decode file. Please save as UTF-8 and try again.",
                        reply_markup=InlineKeyboardMarkup(cancel_keyboard)
                    )
                    return PRODUCT_KEYS

            # Normalize line endings, split, strip whitespace, drop blank lines
            lines = [l.strip()
                     for l in text.replace('\r\n', '\n').replace('\r', '\n').split('\n')]
            keys = [l for l in lines if l]
            if ptype == ProductType.ACCOUNT_LOGIN:
                from services.inventory_import import parse_account_inventory
                keys = parse_account_inventory(text)

            if not keys:
                await update.message.reply_text(
                    f"❌ No valid {item_label}s found in file "
                    f"(empty or all blank lines).\n"
                    f"Try again, paste {item_label}s directly, or type 'skip':",
                    reply_markup=InlineKeyboardMarkup(cancel_keyboard)
                )
                return PRODUCT_KEYS
            ...
            _log.getLogger(__name__).warning("product_keys file upload error: %s", type(e).__name__)
            await update.message.reply_text(
                f"❌ Error reading file. Please try again, "
                f"paste {item_label}s directly, or type 'skip':",
                reply_markup=InlineKeyboardMarkup(cancel_keyboard)
            )
            return PRODUCT_KEYS
    ...
    # ── Text paste path ───────────────────────────────────────────────────────
        ...
        if raw_text.strip().lower() == 'skip':
            context.user_data['product_keys'] = []
            await create_product_final(update, context)
            return ConversationHandler.END

        # Normalize line endings, split, strip, drop blanks — no split on ":"
        lines = [l.strip()
                 for l in raw_text.replace('\r\n', '\n').replace('\r', '\n').split('\n')]
        keys = [l for l in lines if l]
        if ptype == ProductType.ACCOUNT_LOGIN:
            from services.inventory_import import parse_account_inventory
            keys = parse_account_inventory(raw_text)

        if not keys:
            await update.message.reply_text(
                f"❌ No valid {item_label}s found. "
                f"Please paste {item_label}s (one per line), "
                f"upload a .txt file, or type 'skip':",
                reply_markup=InlineKeyboardMarkup(cancel_keyboard)
            )
            return PRODUCT_KEYS
    ...
    # ── Unrecognised input (sticker, voice, video, etc.) ─────────────────────
    else:
        await update.message.reply_text(
...
async def create_product_final(update, context):
    ...
    # Guard: if conversation was lost (e.g. bot restart mid-flow), abort gracefully
    _required = ('product_name', 'product_desc', 'product_price', 'product_type', 'product_category')
    if any(k not in context.user_data for k in _required):
        msg = "❌ Session expired. Please start product creation again (/admin → Products → Create)."
        try:
            if update.message:
    ...
    with get_db_session() as session:
        ...
        # V11 — types that don't use ``product_keys`` inventory should not
        # show 0 stock (breaks the buy button); use unlimited sentinel.
        UNLIMITED_TYPES = {
            ProductType.FILE, ProductType.DOWNLOADABLE_FILE,
            ProductType.AUTO_GENERATED, ProductType.MANUAL_DELIVERY,
            ProductType.PREORDER, ProductType.SUBSCRIPTION,
            ProductType.SERVICE, ProductType.EXTERNAL_DELIVERY,
            ProductType.BUNDLE,
        }
        if context.user_data['product_type'] in UNLIMITED_TYPES:
            stock_count = 999999
        ...
        # Snapshot attributes NOW — any later commit expires them and, under
        # asyncio + scoped_session, a concurrent handler can even detach the
        # instance ("not bound to a Session"). Read once, use locally.
        p_id = product.id
        p_name = product.name
        ...
        # V23: Record initial price in price-history (best-effort, non-blocking)
        try:
            from services.price_history_service import record_price_change
            _admin_name = None
            _admin_tg   = None
            if update.effective_user:
                _admin_tg   = update.effective_user.id
                _admin_name = (update.effective_user.full_name or
                               update.effective_user.username or str(_admin_tg))
            record_price_change(
                product_id=p_id,
                old_price=0.0,
                new_price=p_price,
                changed_by_telegram_id=_admin_tg,
                changed_by_name=_admin_name,
                reason="initial",
            )
        except Exception:
            pass

        # V11 — KEY-like inventory types share the same product_keys upload
        KEY_LIKE = {
            ProductType.KEY, ProductType.REDEEM_LINK,
            ProductType.ACCOUNT_LOGIN, ProductType.VOUCHER,
        }
        ...
        if product_keys and context.user_data['product_type'] in KEY_LIKE:
            ...
            duplicates_skipped = len(dupes) + len(invalid)
            if accepted:
                session.bulk_save_objects([
                    ProductKey(product_id=p_id, key_value=k,
                               key_fingerprint=fp, is_sold=False)
                    for k, fp in accepted
                ])
                session.commit()
                keys_added = len(accepted)
                session.query(Product).filter_by(id=p_id).update(
                    {Product.stock_count: keys_added}, synchronize_session=False,
                )
                session.commit()
...
Product ID: #{p_id}"""
        ...
        if keys_added > 0:
            message += f"\n📥 Inventory added: {keys_added}"
        elif context.user_data['product_type'] in KEY_LIKE:
            message += "\n\n⚠️ No inventory added. Use the Restock option to add later."
        if duplicates_skipped:
            message += f"\n⚠️ Duplicates skipped: {duplicates_skipped}"

        # Nudge admin to finish per-type configuration when applicable.
        needs_config = context.user_data['product_type'] in {
            ProductType.AUTO_GENERATED, ProductType.SUBSCRIPTION,
            ProductType.BUNDLE, ProductType.SERVICE,
            ProductType.EXTERNAL_DELIVERY, ProductType.PREORDER,
            ProductType.DOWNLOADABLE_FILE,
        }
        if needs_config:
            message += (
                "\n\nℹ️ Per-type settings (plans / bundle items / integration / "
                "generation rules / customer-info fields / Telegram file_id) "
                "can be added from the admin control center."
            )
    ...
    # Channel Auto-Post (V18): best-effort post of the new product to the
    # configured channel. Runs after the DB session above is closed (it
    # opens its own) and never raises — a channel-posting failure must
    # never break product creation.
    try:
        from services.channel_poster import post_new_product
        await post_new_product(context.bot, p_id)
    except Exception:
        import logging as _log
        _log.getLogger(__name__).exception(
            "channel auto-post failed for new product_id=%s", p_id)
    ...
    with get_db_session() as session:
        ...
        if total_pages > 1:
            ...
            if page < total_pages - 1:
                pagination_row.append(InlineKeyboardButton("Next »", callback_data=f"admin_edit_category_page_{page+1}"))
...
async def edit_subcategory_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with get_db_session() as session:
        ...
        if total_pages > 1:
            ...
            pagination_row.append(InlineKeyboardButton(f"Page {page+1}/{total_pages}", callback_data="noop"))
            if page < total_pages - 1:
                pagination_row.append(InlineKeyboardButton("Next »", callback_data=f"admin_edit_subcategory_page_{page+1}"))
            keyboard.append(pagination_row)
...
async def edit_product_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with get_db_session() as session:
        # Get all products (exclude soft-deleted ones - see edit_delete below)
        all_products = (
        ...
        if total_pages > 1:
            ...
            pagination_row.append(InlineKeyboardButton(f"Page {page+1}/{total_pages}", callback_data="noop"))
            if page < total_pages - 1:
                pagination_row.append(InlineKeyboardButton("Next »", callback_data=f"admin_edit_product_page_{page+1}"))
            keyboard.append(pagination_row)
...
async def edit_select_field(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "edit_delete":
        # Soft-delete the product only - order items remain untouched.
        #
        # NOTE: order_items.product_id is NOT NULL, so we must never call
        # session.delete(product) here. Deleting the Product row would make
        # SQLAlchemy try to null out product_id on every related OrderItem
        # (via the Product.order_items / OrderItem.product relationship),
        # which violates the NOT NULL constraint and raises
        # IntegrityError. Marking the product hidden/inactive instead keeps
        # its id valid for existing OrderItem rows and preserves order
        # history, while removing it from the catalog and admin pickers.
        with get_db_session() as session:
            ...
            # Soft-delete the product: keep the row so order_items.product_id
            # stays valid and non-null; hide it everywhere is_active is checked.
            product.is_active = False
    ...
    with get_db_session() as session:
        ...
        if field == 'name':
            ...
            current_emoji = product.product_emoji or "📦 (default)"
            prompt = (
                f"🙂 Current emoji: {current_emoji}\n\n"
                f"Send a single emoji to use for this product in the catalog "
                f"(e.g. 🐦, 🌈, 🎬), or send \"none\" to clear it and use the default 📦:"
            )
...
async def edit_new_value(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with get_db_session() as session:
        ...
        if field == 'name':
            ...
            product.product_emoji = None if cleaned.lower() in ("none", "-", "clear") else cleaned[:32]
        elif field == 'price':
            _old_price = _new_price = _price_changed_product_id = None  # pre-init
            try:
                _old_price = float(product.price)
                product.price = float(new_value)
                _new_price = float(new_value)
                _price_changed_product_id = product.id
            except ValueError:
                cancel_keyboard = [[InlineKeyboardButton("❌ Cancel", callback_data="cancel_edit")]]
                await update.message.reply_text(
        ...
        # V23: Record price change in price-history (best-effort, non-blocking)
        if (field == 'price'
                and _old_price is not None
                and _new_price is not None
                and _old_price != _new_price):
            try:
                from services.price_history_service import record_price_change
                _admin_tg   = update.effective_user.id if update.effective_user else None
                _admin_name = None
                if update.effective_user:
                    _admin_name = (update.effective_user.full_name or
                                   update.effective_user.username or str(_admin_tg))
                record_price_change(
                    product_id=_price_changed_product_id,
        ...
        # V18 — fire price-drop notifications after commit so DB is consistent
        if (field == 'price'
                and _old_price is not None
                and _new_price is not None
                and _new_price < _old_price):
            try:
                from services.price_alerts import notify_price_drop_async
                await notify_price_drop_async(
                    context.bot,
                    _price_changed_product_id,
                    _old_price,
                    _new_price,
                )
            except Exception:
                pass  # Notifications must never break the admin flow
            ...
            with get_db_session() as session:
                ...
                if product.image_path and os.path.exists(product.image_path):
                    try:
                        ...
                        logging.getLogger(__name__).warning("Error deleting old image: %s", e)
        ...
        if settings.store_logo_path and os.path.exists(settings.store_logo_path):
            try:
                ...
                logging.getLogger(__name__).debug(
                    "Could not remove old store logo %s: %s", settings.store_logo_path, e
                )
