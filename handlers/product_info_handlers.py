...
async def admin_product_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # V49: Premium Product System — global Button Builder and Tag catalog
    rows.append([
        InlineKeyboardButton("🔘 Button Builder", callback_data="btnb:list"),
        InlineKeyboardButton("🏷 Tags Catalog", callback_data="ptag:catalog"),
    ])
...
async def admin_product_blocks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # V49: Premium Product System — per-product Feature Box and Tags
    rows.append([
        InlineKeyboardButton("⭐ Feature Box", callback_data=f"fbx:list:{product_id}"),
        InlineKeyboardButton("🏷 Tags", callback_data=f"ptag:assign:{product_id}"),
    ])
...
def build_pib_conversation() -> ConversationHandler:
    ...
    return ConversationHandler(
        ...
        fallbacks=[
            CallbackQueryHandler(_pib_conv_end, pattern=r"^pib:admin:prod:\d+$"),
            CallbackQueryHandler(_pib_conv_end, pattern=r"^pib:admin:tpl:0$"),
            MessageHandler(filters.COMMAND, _pib_conv_end),
        ],
        per_user=True,
        per_chat=True,
        allow_reentry=True,
