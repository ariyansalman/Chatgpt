...
async def lam_sessions(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton("« Prev", callback_data=f"{base}:{page - 1}"))
    nav.append(InlineKeyboardButton(f"{page + 1}/{total_pages}", callback_data="lam:noop"))
    if page < total_pages - 1:
        nav.append(InlineKeyboardButton("Next »", callback_data=f"{base}:{page + 1}"))
    if len(nav) > 1:
        kb_rows.append(nav)
...
async def lam_user(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if page > 0:
        nav.append(InlineKeyboardButton(
            "« Prev", callback_data=f"lam:user:{db_uid}:{page - 1}"))
    nav.append(InlineKeyboardButton(f"{page + 1}/{total_pages}", callback_data="lam:noop"))
    if page < total_pages - 1:
        nav.append(InlineKeyboardButton(
            "Next »", callback_data=f"lam:user:{db_uid}:{page + 1}"))
    if len(nav) > 1:
        kb_rows.append(nav)
...
async def lam_devices(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if page > 0:
        nav.append(InlineKeyboardButton(
            "« Prev", callback_data=f"lam:devices:{page - 1}"))
    nav.append(InlineKeyboardButton(f"{page + 1}/{total_pages}", callback_data="lam:noop"))
    if page < total_pages - 1:
        nav.append(InlineKeyboardButton(
            "Next »", callback_data=f"lam:devices:{page + 1}"))
    if len(nav) > 1:
        kb_rows.append(nav)
