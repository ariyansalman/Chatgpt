...
def _payment_kb(tx) -> InlineKeyboardMarkup:
    ...
    kb.append([InlineKeyboardButton("🔙 Back", callback_data="mp:list:0:desc")])
...
async def payments_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    pag = []
    if page > 0:
        pag.append(InlineKeyboardButton("« Prev", callback_data=f"mp:list:{page-1}:{sort}"))
    if total_pages > 1:
        pag.append(InlineKeyboardButton(f"{page+1}/{total_pages}", callback_data="noop"))
    if page < total_pages - 1:
        pag.append(InlineKeyboardButton("Next »", callback_data=f"mp:list:{page+1}:{sort}"))
    if pag:
        kb.append(pag)
...
async def payment_confirm_execute(update: Update, context: ContextTypes.DEFAULT_TYPE):
                ...
                try:
                    await query.edit_message_text(
                        ...
                        reply_markup=InlineKeyboardMarkup(
                            [[InlineKeyboardButton("🔙 Back", callback_data=f"mp:det:{tx_id}")]]
                        ),
                    )
                except BadRequest as e:
                    if "Message is not modified" not in str(e):
    ...
    with get_db_session() as session:
        ...
        kb  = _payment_kb(tx) if tx else InlineKeyboardMarkup(
            [[InlineKeyboardButton("🔙 Back", callback_data="mp:list:0:desc")]]
        )
