...
def _user_info_kb(user) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🔙 Back", callback_data="usr:list:0:desc")],
    ])
...
async def users_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    pag = []
    if page > 0:
        pag.append(InlineKeyboardButton("« Prev", callback_data=f"usr:list:{page-1}:{sort}"))
    if total_pages > 1:
        pag.append(InlineKeyboardButton(f"{page+1}/{total_pages}", callback_data="noop"))
    if page < total_pages - 1:
        pag.append(InlineKeyboardButton("Next »", callback_data=f"usr:list:{page+1}:{sort}"))
    if pag:
        kb.append(pag)
...
async def balance_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🔙 Back",         callback_data=f"usr:det:{user_id}")],
    ])
    try:
        try:
            await query.edit_message_text(
...
async def purchase_history(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    pag = []
    if page > 0:
        pag.append(InlineKeyboardButton("« Prev", callback_data=f"usr:ord:{user_id}:{page-1}"))
    if total_pages > 1:
        pag.append(InlineKeyboardButton(f"{page+1}/{total_pages}", callback_data="noop"))
    if page < total_pages - 1:
        pag.append(InlineKeyboardButton("Next »", callback_data=f"usr:ord:{user_id}:{page+1}"))
    if pag:
        kb.append(pag)
    kb.append([InlineKeyboardButton("🔙 Back", callback_data=f"usr:det:{user_id}")])
