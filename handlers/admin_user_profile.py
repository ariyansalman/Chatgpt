...
async def up_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton("« Prev", callback_data=f"up:list:{page-1}:{sort}"))
    nav.append(InlineKeyboardButton(f"{page+1}/{total_pages}", callback_data="noop"))
    if page + 1 < total_pages:
        nav.append(InlineKeyboardButton("Next »", callback_data=f"up:list:{page+1}:{sort}"))
    if nav:
        kb_rows.append(nav)
...
async def up_orders(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with get_db_session() as session:
        ...
        # Pagination nav
        if page > 0:
            nav.append(InlineKeyboardButton("« Prev", callback_data=f"up:ord:{uid}:{page-1}"))
        nav.append(InlineKeyboardButton(f"{page+1}/{total_pages}", callback_data="noop"))
        if page + 1 < total_pages:
            nav.append(InlineKeyboardButton("Next »", callback_data=f"up:ord:{uid}:{page+1}"))
...
async def up_topup_history(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with get_db_session() as session:
        ...
        if page > 0:
            nav.append(InlineKeyboardButton("« Prev", callback_data=f"up:topup:{uid}:{page-1}"))
        nav.append(InlineKeyboardButton(f"{page+1}/{total_pages}", callback_data="noop"))
        if page + 1 < total_pages:
            nav.append(InlineKeyboardButton("Next »", callback_data=f"up:topup:{uid}:{page+1}"))
...
async def up_referrals(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with get_db_session() as session:
        ...
        if page > 0:
            nav.append(InlineKeyboardButton("« Prev", callback_data=f"up:ref:{uid}:{page-1}"))
        nav.append(InlineKeyboardButton(f"{page+1}/{total_pages}", callback_data="noop"))
        if page + 1 < total_pages:
            nav.append(InlineKeyboardButton("Next »", callback_data=f"up:ref:{uid}:{page+1}"))
...
async def up_wallet_history(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with get_db_session() as session:
        ...
        if page > 0:
            nav.append(InlineKeyboardButton("« Prev", callback_data=f"up:wal:{uid}:{page-1}"))
        nav.append(InlineKeyboardButton(f"{page+1}/{total_pages}", callback_data="noop"))
        if page + 1 < total_pages:
            nav.append(InlineKeyboardButton("Next »", callback_data=f"up:wal:{uid}:{page+1}"))
...
async def up_coupons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with get_db_session() as session:
        ...
        if page > 0:
            nav.append(InlineKeyboardButton("« Prev", callback_data=f"up:coup:{uid}:{page-1}"))
        nav.append(InlineKeyboardButton(f"{page+1}/{total_pages}", callback_data="noop"))
        if page + 1 < total_pages:
            nav.append(InlineKeyboardButton("Next »", callback_data=f"up:coup:{uid}:{page+1}"))
