...
async def gcn_toggle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    A network cannot be enabled if:
      • Address is empty
      • Address format is invalid (too short)
    When blocked, the status is shown as ⚪ Not Configured.
    """
    query = update.callback_query
    await query.answer()
    if not has_permission(update.effective_user.id, "manage_payments"):
    ...
    currently_active = n.get("is_active", True)

    if not currently_active:
        # ── Validate before enabling ──────────────────────────────────────
        address = (n.get("address") or "").strip()
        if not address:
            await query.answer(
                f"⚪ Cannot enable \"{n.get('name', 'this network')}\".\n"
                "No deposit address is configured.\n"
                "Please set an address first using ✏️ Edit Address.",
                show_alert=True,
            )
            return
        if len(address) < 10:
            await query.answer(
                f"⚪ Cannot enable \"{n.get('name', 'this network')}\".\n"
                "The address appears invalid (too short).\n"
                "Please set a valid address first.",
                show_alert=True,
            )
            return

    n["is_active"] = not currently_active
    _save(gw, networks)
