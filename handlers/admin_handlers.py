...
async def admin_view_users_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with get_db_session() as session:
        ...
        if total_pages > 1:
            ...
            if page < total_pages - 1:
                pagination_row.append(InlineKeyboardButton("Next »", callback_data=f"admin_view_users_page_{page+1}"))
...
async def admin_view_orders_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with get_db_session() as session:
        ...
        if total_pages > 1:
            ...
            if page < total_pages - 1:
                pagination_row.append(InlineKeyboardButton("Next »", callback_data=f"admin_view_orders_page_{page+1}"))
            if pagination_row:
                keyboard.append(pagination_row)
...
async def admin_complete_order_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    Note: the ``transition()`` call below opens its own
    ``get_db_session()`` from inside this function's session block, same
    nested-session shape as the payment/order callbacks fixed elsewhere in
    this file — but ``transition()`` is a shared utility used by many
    other callers throughout the codebase, so fixing it here is out of
    scope for this pass; flagging it rather than changing shared behavior
    blind. This function makes no writes of its own via `session` (only a
    read), so there's nothing of this function's own to lose — only the
    trailing refresh call is moved outside, matching the fix pattern used
    elsewhere.
    """
    query = update.callback_query
    await query.answer()
    ...
    # Refresh order details — fresh session, fresh query.
...
async def admin_confirm_payment_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    ROOT CAUSE FIX — nested get_db_session() call:
    This handler used to call ``await admin_confirm_order_menu(update, context)``
    (its own "refresh the Pending Deposits panel" step) from *inside* this
    function's still-open ``with get_db_session() as session:`` block.
    ``admin_confirm_order_menu`` opens its OWN ``get_db_session()`` — and
    since ``Session`` is a ``scoped_session``, that nested call resolves to
    the exact SAME shared session this function is still using, then closes
    it out from under the caller when its own ``with`` block exits. This is
    the identical anti-pattern ``services/idempotency.py`` explicitly warns
    against for ``claim()`` vs ``claim_locked()`` — it applies to any call
    that opens a fresh session while one is already open, not just the
    idempotency claim. Net effect: the wallet was credited and the
    "Payment Confirmed" notification was sent (both committed correctly
    before the nested call), but the panel-refresh step that re-queries and
    re-renders the Pending Deposits list ran against a session already
    mid-teardown, so the admin could still see the just-approved deposit
    sitting in the list.

    Fix: do ALL database work in a single, self-contained
    ``with get_db_session()`` block, let it commit and close cleanly, and
    only THEN (outside the block) refresh the Pending Deposits panel via a
    brand-new session — the same pattern already used everywhere else in
    this project (see handlers/admin_pending_deposits.py
    deposit_approve_execute). Also now records ``approved_by`` on the
    transaction and always refreshes the panel (instead of just a toast)
    on the "already processed" / "not found" paths too, so the
    Confirm/Cancel buttons never stay live for a deposit that's already
    resolved.
    """
    query = update.callback_query
    await query.answer()
    ...
    # Outcome flags/values — decided inside the DB session, acted upon
    # (notifications, panel refresh) only after it has cleanly committed
    # and closed.
    outcome: str = "error"          # "ok" | "not_found" | "already" | "error"
    ...
    with get_db_session() as session:
        ...
        if not txn:
            ...
            # Idempotency guard — stable reference is the transaction's own
            # DB id (never the Telegram update_id, which changes on
            # button-tap redelivery/retry). Defense-in-depth alongside the
            # atomic conditional UPDATE below: if the claim itself raises,
            # fail CLOSED (no credit).
            #
            # Uses claim_locked() (not claim()) because we are already
            # inside this open get_db_session() block — see the fix note
            # in the docstring above for why that matters everywhere, not
            # just here.
            try:
                from services.idempotency import claim_locked as _idem_claim_locked
                claimed = _idem_claim_locked(session, "admin_approve", f"tx:{txn_id}")
            except Exception:
            ...
            if outcome != "error":
                if not claimed:
                    ...
                    # Atomically flip PENDING → COMPLETED so a double-click
                    ...
                    if flipped == 0:
                        ...
                        log_admin_action(
                            admin_tg_id, "payment.approve",
                            target_type="transaction", target_id=txn_id,
                            details=f"user_id={txn.user_id} amount={amount}",
                        )

    # ── Everything below runs AFTER the session above has committed and
    # closed cleanly — no more DB work is nested inside another open
    # session, so the Pending Deposits panel always reflects the live,
    ...
    if outcome == "already":
        ...
        # Refresh the panel too (not just a toast) so a stale Confirm/Cancel
        # button never lingers on an already-resolved deposit.
    ...
    # Go back to payment confirmation menu — fresh session, fresh query,
    # guaranteed not to show the deposit we just approved.
    await admin_confirm_order_menu(update, context)
...
async def admin_cancel_payment_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    Same nested get_db_session() fix as admin_confirm_payment_callback above:
    the old code called ``await admin_cancel_order_menu(update, context)``
    (which opens its own session) from inside this function's still-open
    session block. Restructured so all DB work commits and closes cleanly
    first, and the menu refresh + user notification happen afterward.
    """
    query = update.callback_query
    await query.answer()
    ...
    outcome: str = "ok"   # "ok" | "not_found" | "already"
    user_telegram_id: int | None = None
    amount: float = 0.0
    ...
    with get_db_session() as session:
        ...
        if not txn:
            ...
            # cancellation, not a payment failure, and CANCELLED is what
            # frees the user to create a new pending order right away.
    ...
    # ── Runs only after the session above has cleanly committed and
    # closed — the refresh below always reflects the live, just-committed
    # state, never a stale/mid-teardown one. ────────────────────────────
    if outcome == "not_found":
        try:
            await query.edit_message_text("❌ Transaction not found.")
        except BadRequest as e:
    ...
    if outcome == "already":
        await query.answer("⚠️ Already processed.", show_alert=True)
        # Refresh the panel too, so a stale Cancel/Delete button never
        # lingers on an already-resolved deposit.
    ...
    # Go back to payment cancellation menu — fresh session, fresh query.
    await admin_cancel_order_menu(update, context)
...
async def admin_delete_payment_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    Same nested get_db_session() fix as the confirm/cancel callbacks above:
    ``admin_cancel_order_menu`` (which opens its own session) used to be
    called from inside this function's still-open session block.
    """
    query = update.callback_query
    await query.answer()
    ...
    with get_db_session() as session:
        ...
        if not txn:
            ...
            log_admin_action(update.effective_user.id, "payment.delete",
                             target_type="transaction", target_id=txn_id,
                             details=f"user_id={deleted_user_id if user else 'unknown'} amount={amount}")
...
async def admin_cancel_order_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    Same nested get_db_session() fix as the payment confirm/cancel/delete
    callbacks above: ``admin_order_detail_callback`` (which opens its own
    session) used to be called from inside this function's still-open
    session block, risking a stale refresh of the order detail screen
    right after the refund.
    """
    query = update.callback_query
    await query.answer()
    ...
    outcome: str = "ok"   # "ok" | "not_found" | "not_refundable"
    user_telegram_id: int | None = None
        ...
        if flipped == 0:
            ...
            if not order:
                ...
                log_admin_action(update.effective_user.id, "order.cancel_refund",
                                 ...
                                 details=f"user_id={order.user_id} refund={refund_amount}")
    ...
    # outcome == "ok"
    await query.answer(f"✅ Order cancelled and {format_price(refund_amount)} refunded!", show_alert=True)
